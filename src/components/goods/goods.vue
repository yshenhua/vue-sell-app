<template>
  <div>
            ...
            <ul>
              <li
                ...
                <div class="content">
                  ...
                  <div class="extra">
                    <span class="count">月售{{food.sellCount}}份</span><!-- 清除空白字符
                    --><span>好评率{{food.rating}}%</span>
                  </div>
                  <div class="price">
                    <span class="now">￥{{food.price}}</span><!-- 清除空白字符
    ...
    <food :food="selectedFood" @cart-add="_drop" ref="food"></food>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    this.$http.get('/api/goods').then((res) => {
      ...
      if (res.errno === ERR_OK) {
        this.goods = res.data;
        // Vue的数据更新是一个异步操作，不会更新DOM，这里要用nextTick
        this.$nextTick(() => {
          this._initScroll();
  ...
  methods: {
    ...
    _initScroll () {
      ...
      this.foodsScroll.on('scroll', (pos) => {
        ...
        if (pos.y <= 0) this.scrollY = Math.abs(Math.round(pos.y));
      });
    },
    _calculateHeight () {
      let foodList = this.$refs.foodsWrapper.getElementsByClassName('food-list-hook');
